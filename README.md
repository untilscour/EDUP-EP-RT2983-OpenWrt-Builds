...
4. Allow "Force upgrade" (tick the box) because the name is now "edup,ep-rt2983" as it should have been from the start. 
5. Unselect "Keep settings" - Note: All settings will be reset to default. Wifi is not enabled by default so a connection via Ethernet is necessary to log in and set up. 
